...
> :information_source: The Mocha test suite under `test/basic.js` covers an automated
> matrix of plugin generators pinned to **yeoman-generator** majors 4, 5, 6, 7
> and 8 (one fixture per major under `test/generator-ui5-test-v<N>/`). The
> scenarios below are the manual smoke tests against the live `ui5-community`
> registry and `bestofui5.org` — they are not version-pinned.
